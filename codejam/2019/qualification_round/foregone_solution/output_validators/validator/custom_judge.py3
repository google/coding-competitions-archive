...
def _utils_ToFloat(s):
  ...
  try:
    ...
    return None
"""Custom judge for Foregone Solution, Code Jam 2019."""



_BAD_FORMAT_ERROR = 'Output format is incorrect'
_BAD_SUM = 'Output does not sum up to input'
_BAD_OUTPUT_FOURS = 'Output contains digit 4'
...
def VerifyOutput(lines, n):
  ...
  """
  if not lines or len(lines) != 1 or len(lines[0]) != 2:
...
def FindError(unused_self, input_file, output_file, attempt_file):
  ...
  for case_num, (case, _, attempt_lines) in enumerate(
      zip(input_cases, output_cases, attempt_cases), start=1):
    error = VerifyOutput(attempt_lines, case)
    if error is not None:
