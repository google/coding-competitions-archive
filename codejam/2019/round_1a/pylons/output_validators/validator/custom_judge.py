...
def _utils_ToFloat(s):
  ...
  try:
    ...
    return None
"""Custom judge for Pylons, Code Jam 2019."""
...
_BAD_FORMAT_ERROR = 'Output is not a valid solution or IMPOSSIBLE'
...
_BAD_OUT_OF_BOUNDS_CELL = ('Cell {0} is out of bounds.').format
_BAD_INVALID_MOVE = ('Solution tries to move from {0} to {1}.').format
_BAD_REVISIT_CELL = ('Solution tries to revisit cell {0}.').format
...
"""
_CaseInput = collections.namedtuple('_CaseInput', ['r', 'c'])
...
def ParseInputFile(input_file):
  ...
  cases = []
  while input_lines:
    r, c = map(int, input_lines.pop(0))
    cases.append(_CaseInput(r, c))
  return cases


def IsValidMove(first, second):
  """Checks whether a move from `first` cell to `second` cell is valid."""
  return (first[0] != second[0] and
          first[1] != second[1] and
          first[0] - first[1] != second[0] - second[1] and
          first[0] + first[1] != second[0] + second[1])


def VerifyOutput(lines, case, expected_solution_exists):
  ...
  Args:
    ...
    expected_solution_exists: Whether a solution exists based on our output.
  ...
  if len(lines) == 1:
    ...
    # Either expected_solution_exists is None or False.
    if expected_solution_exists is None or not expected_solution_exists:
      return None, False
  ...
  for line in lines[1:]:
    ...
    if not (line[0].isdigit() and line[1].isdigit()):
      return _BAD_FORMAT_ERROR, None
    cell = tuple(map(int, line))

    if not (1 <= cell[0] <= case.r and 1 <= cell[1] <= case.c):
      return _BAD_OUT_OF_BOUNDS_CELL(cell), None

    if cell in visited_cells:
      return _BAD_REVISIT_CELL(cell), None
    visited_cells.add(cell)

    if last_cell is not None and not IsValidMove(last_cell, cell):
      return _BAD_INVALID_MOVE(last_cell, cell), None
    last_cell = cell

  if expected_solution_exists is None:
    return None, True
  if expected_solution_exists:
    return None, True
