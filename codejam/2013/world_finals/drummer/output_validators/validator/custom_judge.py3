...
class DefaultJudge(object):
  ...
  def __CompareTokens(self, expected, received):
    ...
    for index in range(min(len(expected), len(received))):
    ...
    for index in range(1, min(len(expected), len(received)) + 1):
  ...
  def FindError(self, input_file, output_file, attempt):
    ...
    if len(rows1) < len(rows2):
      rows1 += [[] for _ in range(len(rows2) - len(rows1))]
    elif len(rows2) < len(rows1):
      rows2 += [[] for _ in range(len(rows1) - len(rows2))]
