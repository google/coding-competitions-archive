...
const string kBadJudgeImpossibleClaimError =
    "JUDGE_ERROR Contestant finds a solution while judge claims a solution "
    "does not exist";
const string kInvalidLengthError =
    "Solution length does not match the number of buildings";
const string kInvalidHeightError = "Solution building height is out of range";
const string kWrongInformationError =
    "Solution building does not match with the provided information";
...
string JudgeCase(const CaseInput& input, const CaseOutput& correct_output,
  ...
  if (any_of(attempt.begin(),
             attempt.end(),
             [input] (int h) { return h < 1 || h > input.N; })
  ) {
    return kInvalidHeightError;
  }

  int expected_A = 0;
  int current_max_height = 0;
  for (int i = 0; i < input.N; ++i) {
    if (attempt[i] >= current_max_height) {
      ++expected_A;
      current_max_height = attempt[i];
    }
  }
  ...
  int max_height = *max_element(attempt.begin(), attempt.end());
  int expected_C = count(attempt.begin(), attempt.end(), max_height);

  if (expected_A != input.A || expected_B != input.B || expected_C != input.C) {
    return kWrongInformationError;
  }

  if (correct_output == kImpossibleOutput) {
    return kBadJudgeImpossibleClaimError;
  }
...
void Test() {
  assert(JudgeCase({1, 1, 1, 1}, {1}, kImpossibleOutput) ==
         kBadImpossibleClaimError);
  assert(JudgeCase({1, 1, 1, 1}, kImpossibleOutput, {1}) ==
         kBadJudgeImpossibleClaimError);
  assert(JudgeCase({1, 1, 1, 1}, {1}, {1, 1}) == kInvalidLengthError);
  assert(JudgeCase({1, 1, 1, 1}, {1}, {2}) == kInvalidHeightError);
  assert(JudgeCase({1, 1, 1, 1}, {1}, {1}) == kAccepted);

  assert(JudgeCase({2, 2, 1, 1}, {1, 2}, {1}) == kInvalidLengthError);
  assert(JudgeCase({2, 2, 1, 1}, {1, 2}, {2, 2}) == kWrongInformationError);
  assert(JudgeCase({2, 2, 1, 1}, {1, 2}, {1, 2}) == kAccepted);

  assert(JudgeCase({2, 2, 2, 2}, {1, 1}, {2, 2}) == kAccepted);

  assert(JudgeCase({2, 1, 1, 1}, kImpossibleOutput, kImpossibleOutput) ==
         kAccepted);
}
